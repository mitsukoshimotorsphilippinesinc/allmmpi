...
class Free_of_charge extends Admin_Controller {
	...
	public function for_approval_confirm()
	{
		...
		if (empty($free_of_charge)) {		
			...
			if ($is_approved == 'yes') {
				...
				$html = "You are about to approve the Service Unit with Request Code: <strong>" . $free_of_charge_code . "</strong>. Do you want to continue?";
			} else {
				$html = "<p>You are about to deny the Service Unit with Request Code: <strong>" . $free_of_charge_code . "</strong>. <br/>
	...
	public function for_approval_proceed()
	{
		...
		if (empty($free_of_charge)) {		
			...
			if ($is_approved == 'no') {
				...
				$html = "You have denied the Service Unit Code: <strong>{$free_of_charge_code}</strong>.";
				...
				$html = "You have successfully approved the Service Unit Code: <strong>{$free_of_charge_code}</strong>.";
	...
	public function view_details()
	{
		...
		if (empty($free_of_charge)) {		
			...
			$data = array(
				...
				'free_of_charge_details' =>$free_of_charge_details
			);
		
			$html = $this->load->view("free_of_charge/view_details",$data,true);
	...
	function export_xls($start_date,$end_date)
	{
		...
		try {
			...
			$filename = "free_of_charge_" . str_replace("-", "", $start_date) . "-" . str_replace("-", "", $end_date) . ".xls";
