...
class Service_unit extends Admin_Controller {
	...
	public function for_approval_confirm()
	{
		...
		if (empty($service_unit)) {		
			...
			if ($is_approved == 'yes') {
				...
				$html = "You are about to approve the Service Unit with Request Code: <strong>" . $service_unit_code . "</strong>. Do you want to continue?";
			} else {
				$html = "<p>You are about to deny the Service Unit with Request Code: <strong>" . $service_unit_code . "</strong>. <br/>
	...
	public function for_approval_proceed()
	{
		...
		if (empty($service_unit)) {		
			...
			if ($is_approved == 'no') {
				...
				$html = "You have denied the Service Unit Code: <strong>{$service_unit_code}</strong>.";
				$title = "Denied :: " . $service_unit_code;

				$this->return_json("1","Denied Service Unit.",array("html" => $html, "title" => $title));		
				...
				$html = "You have successfully approved the Service Unit Code: <strong>{$service_unit_code}</strong>.";
				$title = "Approved :: " . $service_unit_code;
			}
	...
	public function view_details()
	{
		...
		if (empty($service_unit)) {		
			...
			$data = array(
				...
				'service_unit_details' =>$service_unit_details
			);
		
			$html = $this->load->view("service_unit/view_details",$data,true);
			 
			$title = "View Details :: " . $service_unit_code;
			$this->return_json("1","View Details Service Unit", array("html" => $html, "title" => $title));
	...
	function export_xls($start_date,$end_date)
	{
		...
		try {
	        ...
	        $objPHPExcel->getProperties()->setTitle("requests")->setDescription("none");
			$start_column_num = 3;
			...
			$filename = "service_unit_" . str_replace("-", "", $start_date) . "-" . str_replace("-", "", $end_date) . ".xls";
			...
			//set column names
			$worksheet->setCellValue('A1', "Service Unit Requests from {$start_date} to {$end_date}");
