...
if (is_object($po))
{
	$isAdd = false;
	$titlePrefix = "Edit&nbsp;";
	$submitURL = "/inventory/orders/edit/" . $po->purchase_order_id."/{$return_url}";
	$_id = $po->purchase_order_number;
	$show_approval = false;
}
else
{
	...
	$submitURL = "/inventory/orders/add/{$source_id}/{$source}";
	$_id = 0;
	$show_approval = true;
}
?-->
...
<div class='alert alert-info'><h3><?= $titlePrefix ?>Warehouse Parts Request <a class='btn return-btn add-close' style='float:right;margin-right:-30px;' >Back to Request List</a></h3></div>

<form id="submit_form" action='<?= $submitURL ?>' method='post' class='form-inline'>
	<fieldset >
		<div class="row-fluid">						
			<div class='alert alert-success'><h4>Requester Details<label class="label label-important request-code-label" id="requester-request-code-label" style="float:right;font-size:16px;">WP</label></h4></div>
		</div>
		...
		<div class="row-fluid">
			<div class="span6">
				...
				<div class="control-group">
					...
					<div class="controls">
						<textarea class='span10' rows="4" placeholder="" name="requester_details" id="requester_details" readonly><?= set_value('requester_details') ?></textarea>
					</div>
				</div>
			...
			<div class="span5">
				...
				<?php
					...
					echo form_dropdown('add_item_warehouse',$warehouse_options, NULL,'id="add_item_warehouse"');
				?>	
				...
				<?php
					...
					foreach ($motorcycle_brandmodel_details as $mbd) {
						$concat_brandmodel =  $mbd->brand_name . ' ' . $mbd->model_name;
					 	$brand_model_options[$concat_brandmodel] = $concat_brandmodel;
					}
					
					echo form_dropdown('add_item_brandmodel',$brand_model_options, NULL,'id="add_item_brandmodel"');
				?>
		...
		<br/>
		<div class='alert alert-success'><h4>Items<label class="label label-important request-code-label" id="item-request-code-label" style="float:right;font-size:16px;">WP</label></h4></div>
		...
		<div class="row-fluid span8">
			...
			<table class="table inventory-order-items table-bordered table-striped">
				...
				<tbody id="wr_items">
					...
					//if ($isAdd)
					//{
					//	$temp = $this->input->post('add_item_good_qty');
					//}
					//else
					//{
					//	$temp = json_decode($po->json_items, true);
					//}
					...
					<tr class="item_row">
						...
						<td class="qty"><?=number_format(set_value('item_good_qty['.$i.']',$temp[$i]['good_qty']))?></td>
						<td class="qty"><?=number_format(set_value('item_bad_qty['.$i.']',$temp[$i]['bad_qty']))?></td>
						<td class="price"><?=number_format(set_value('item_price['.$i.']',$temp[$i]['unit_price']),2)?></td>
						<td class="discount"><?=number_format(set_value('discount['.$i.']',$temp[$i]['discount']))?></td>
						<td class="discount_price"><?=number_format(set_value('discount_price['.$i.']',$temp[$i]['discount_price']),2)?></td>
						<td class="remark"><?=set_value('item_remarks['.$i.']',$temp[$i]['remarks'])?></td>
						<td><a class="btn btn-danger rmv_po_item"><i class="icon-white icon-minus"></i></a></td>
						<td class="hidden_values">
							<input type="hidden" name="item_qty[]" id="item_qty[]" value="<?=set_value('item_qty['.$i.']',$temp[$i]['qty'])?>">
							<input type="hidden" name="unit_name[]" id="unit_name[]" value="<?=set_value('unit_name['.$i.']',$unitInfo->unit_name)?>">
							<input type="hidden" name="item_unit[]" id="item_unit[]" value="<?=set_value('item_unit['.$i.']',$temp[$i]['unit_id'])?>">
							<input type="hidden" name="item_name[]" id="item_name[]" value="<?=set_value('item_name['.$i.']',$temp[$i]['item_id'])?>">
							<input type="hidden" name="temp_item[]" id="temp_item[]" value="<?=set_value('temp_item['.$i.']',$itemInfo->item_name)?>">
							<input type="hidden" name="item_price[]" id="item_price[]" value="<?=set_value('item_price['.$i.']',$temp[$i]['unit_price'])?>">
...
<script type="text/javascript">
	...
	var item_entry_row = _.template('\
		...
		<td data="<%= item_id %>"><a class="btn btn-danger rmv_wr_item"><i class="icon-white icon-minus"></i></a></td>\n\
		<td class="hidden_values">\n\
	...
	$(".rmv_wr_item").live('click',function(){
		...
		var item_id = $(this).parent().attr("data");
											...
											proceedRemoveItemModal = b.modal.new({
												...
												buttons: {
													'Ok' : function() {
														...
														$(".rmv_wr_item").parent().parent().remove();
													}
												}
											});
